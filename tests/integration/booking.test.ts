...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it('should respond with status 404 with a invalid body - theres not roomId', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
...
describe("UPDATE /booking/:bookingId", () => {
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    const token = faker.lorem.word();
    const validBody = createValidBody();
    const response = await server.put("/booking/bookingId").set("Authorization", `Bearer ${token}`).send(validBody);
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 with a invalid body if roomId doesn't exist", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
